...
'    TheString = Replace(TheString, "{BoardID}", ID)
  ...
  TheString = ""
  '-------------0-----1-----------2--------------3-----------4----------5------------------6------------------7
  sql= "select [ID], [BBSTitle], [BBSPostUser], [BBSReply], [BBSView], [BBSLastPostTime], [BBSLastPostUser], [BBSBoardID] from [MyBBS_BBS] Where [BBSBoardID]=" & BoardID & " and [BBSRoot]=0 order by [BBSLastPostTime] DESC,  [ID] DESC"
  rs.open sql, conn, 1, 3
  SqlQueryNum	= SqlQueryNum + 1
  if err.number <> 0 then
    response.write "数据库操作失败，请稍候重试！"
    ...
    dim ID, BBSTitle, BBSPostUser, BBSReply, BBSView, BBSLastPostTime, BBSLastPostUser, BBSBoardID
	...
	do while not (rs.bof or rs.eof) and count<Rs.PageSize
	  ...
	  TheString = ""
	  TheString = Replace(Skin_ListSecond, "{BBSTitle}"	,	"<a href='ListView.asp?ID=" & ID & "&BBSBoardID=" & BBSBoardID & "'>" & BBSTitle & "</a>")
